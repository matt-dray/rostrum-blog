...
I used the {rtweet}, {magick} and {sf} R packages to reduce random satellite images of London to a single representative colour and then mapped and tiled them.

# Green/grey belt

I created the [\@londonmapbot](https://twitter.com/londonmapbot) Twitter bot to [tweet out a random satellite image of Greater London](https://www.rostrum.blog/2020/09/21/londonmapbot/) and more recently I've written some posts involving R and abstract art (like [pixel art](https://www.rostrum.blog/2021/06/28/pixel-art/) and [a Shiny app to remix art by Sol LeWitt](https://www.rostrum.blog/2021/07/05/recreate-lewitt/)).
...
This is relatively trivial with the packages {rtweet} to pull tweets and {magick} to manipulate the images.
...
Now we can crop out the logos, quantize the image and resize it using functions from [the {magick} package](https://cran.r-project.org/web/packages/magick/vignettes/intro.html). 

[Quantization is the algorithmic simplification of an image to the colours that best represent it](https://en.wikipedia.org/wiki/Color_quantization). You could, for example, use this for reducing the number of colours in an image to making it easier to compress while minimising information loss. We're going to quantize to just one colour to find the colour that best represents the image. Note that this isn't the same as 'taking an average colour'.
...
Then it's a case of adding these to a map, which in this case is a {ggplot2} object.
...
For something more abstract, we can create squares of each colour and tile them. 
...
[^api]: I'm being relatively [polite](https://www.rostrum.blog/2019/03/04/polite-webscrape/) by doing this, it's probably note strictly necessary.
