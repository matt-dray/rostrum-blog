...
This is sometimes called 'static code analysis', in that you can interrogate the code for syntax errors _before_ it executes. Packages like [{lintr}](https://lintr.r-lib.org/) can even help tidy up ('lint') your code by adjusting or replacing the tokens.

You may also enjoy [a post by Kun Ren](https://renkun.me/2020/11/08/using-parse-data-to-analyze-r-code/) about how this approach is useful for static analysis in [the {languageserver} package](https://github.com/REditorSupport/languageserver).
...
As a result, the dataframe `x` is now in our environment and contains only rows of the `mtcars` that have 8 `carb`uretors.[^carb] So we have the power to delay code execution. Jeepers!

That's great, but we can also pick apart the frozen expression into its constituent tokens. This is where `getParseData()` comes in.

The function takes the expression object as the input and returns a dataframe with one token per row and several columns of handy information. 
...
If you're wondering, the `keep.source = TRUE` bit is needed so that `parse()` returns output in this non-interactive blog post.
...
In fact, this is how R recognises ['infix operators'](https://adv-r.hadley.nz/functions.html?q=infix%20operator#infix-functions) that are bound by percent symbols. This is some special syntactical magic that lets you put the function name _between_ two arguments. So `x %>% head` is equivalent to `` `%>%`(mtcars, head) ``.
...
Not that surprising: it's recognised as `PIPE` and not a `SPECIAL`.

Okay, so we've seen how R parses tokens, what about how it actually executes this code? One way to see this is to look at an 'abstract syntax tree' with [the {lobstr} package](https://lobstr.r-lib.org/).[^lobstr] A 'tree' to show the nested structure of code, with its functions within functions and so on.
