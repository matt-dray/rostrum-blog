...
R is capable of reading R code. Obviously. You can use `getParseData(parse())` to see what's going on. A very simple intro.
...
Another example: the tokens `<-` and `=` have an equivalent role in `x <- 1` and `x = 1`. For style reasons, you might want to find-and-replace `=` with `<-` in your scripts.[^down] Except you shouldn't because `=` is context dependent. In `x = subset(mtcars, subset = carb == 8)`, it's used to assign (`=`), to set a function argument (`=`) _and_ as part of the equivalence operator (`==`).
...
This is sometimes called 'static code analysis', in that you can interrogate the code for syntax errors _before_ it executes. Another example is in the {lintr} package, which can tidy up ('lint') your code by adjusting or replacing the tokens.
...
If you want to take a look at a given R string you can use this little function that contains parse() and getParseData() and returns you the simplified dataframe I showed above if `simplify = TRUE`, otherwise it gives the full read out. An exercise for the reader is to alter this function to accept an R script file rather than a string (hint: `parse()` takes a `file` argument).
...
[^down]: Of course, I'm not mentioning right assignment (`->`) here, not the plucky upstart of [down-asignment](https://www.rostrum.blog/2022/06/07/assign-down/), which is certain to be the future for assignment in R.
...
[^carb]: 'Non-car people' should take a look at the 'Format' section of `?mtcars`. I mean, `drat` means 'rear axle ratio', lol.
...
[^end]: Yeah, I'm hoping you didn't read this far. Obviously I didn't know how to end the post.