...
* one of `Time` in seconds, `Distance` in metres or `Status` ('OK')

# Results

All our requests returned 'OK' for `Status` so there were no problems wth fetching the data.

The data is returned in a tidy data frame.

```{r distance}
nba_travel$Distance
```

We can make these data more useable by making a distance (kilometres) look-up between origin teams (rows) and destination teams (columns).

```{r distance-lookup, message=FALSE}
# So we can spread out data into 'wide format'
library(tidyr)

# Create a mini-lookup of search strings to team codes
