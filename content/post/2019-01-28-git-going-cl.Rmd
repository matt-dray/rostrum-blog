...
As a result, I'm going to consolidate some of my knowledge about a simple Git and GitHub workflow. 

I'm splitting this into two posts: one introducing the basics of the command line -- a necessary first step -- and one to introduce the basics of Git.
...
We're going to be writing Git commands at **the command line**. Before we get to Git, we need to know what the command line is and get some experience of working there.

Most computer users will interact with their machines using a Graphical User Interface (GUI) composed of windows that they can navigate primarily with the mouse. Files and folders can be created with clicks and moved around by dragging. Classic computer stuff that's been around now for decades.

But there's another way: you can write text commands to your computer directly. One particular benefit is that you can save and re-execute commands in a script, which is much more reproducible than an unkown series of mouse-based interactions with your GUI.

The scripting langage used by default is **bash**.

## Access

Open the Terminal application to chat directly to your computer. You can tap command and spacebar to open Spotlight search if you don't know where it is.

Terminal gives us a place to access the command line. It looks prety simple, with just some lines of monospaced text. We'll write our instructions at the **prompt**, which is the form `<computer name>:<current directory> <user>$`.
...
overalls.jpg  plumbing-invoice.txt  warp-pipes.txt
```

## Delete

```
mac:plumbing luigi$ rm warp-pipes.txt
mac:plumbing luigi$ ls
