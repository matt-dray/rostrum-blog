...
[The {keypress} package by Gábor Csárdi](https://github.com/gaborcsardi/keypress) records input from a simple keyboard-button press. You can use this to control games, like the ones in [the tiny {hokey} package](https://github.com/matt-dray/hokey).
...
Problem. In {ActionSquirrel} you move a character around a 2D grid. To go up you type `x$move("up")`, which means 'apply the `move` method to the previously-initialised R6 object called `x`, and supply to the `where` argument the direction `"up"`'. A bit long-winded, eh?
...
Sure, R's `readline()` can take user input, but you would literally have to type `l`, `e`, `f`, `t`, and then hit 'enter', because the function doesn't recognise key presses directly.

This is where [Gábor Csárdi's {keypress} package](https://github.com/gaborcsardi/keypress) comes in. It can accept single button presses from the keyboard, including the arrow keys. The user is prompted and a left-arrow press results in the text `"left"`, for example. 
...
As prompted, move with the arrow keys. If you go too far left, you 'bump' into the end and bounce back. See if you can reach the far-right end. You can do it!

Here's what happens if you input the left-arrow key twice, then multiple presses of the right arrow key. The symbol on the end explains what's happened (`<` is left, `>` is right, `x` is an illegal move, `!` is a win).
...
Such graphics! Such dimensons!

## 3. A clicker

Ever heard of [Cookie Clicker](https://en.wikipedia.org/wiki/Cookie_Clicker)? It's a game where you click. A cookie. A whole bunch of times. Like, seriously, a while bunch of times. Why? To _win_, of course.

Here's a keypress equivalent where you tap keys to vanquish randomised foes. Are there upgrades? No. Are there cool sprites? Not really. But do you click a lot? Also no, but you get to _press buttons_ a lot.

So, initiate a battle with `hokey::battle()` and you're faced with monstrous foes, who have randomised faces made of letters and symbols.
...
Tap keys to deplete the foe's hit points (HP) until they're defeated. Each hit is printed as a period. 
