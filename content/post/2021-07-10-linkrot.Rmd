...
I wrote a function to checkweb pages for [link rot](https://en.wikipedia.org/wiki/Link_rot) and put it in [the R package {linkrot}](https://github.com/matt-dray/linkrot).
...
I've written the function `detect_rot()` to do this for any given web page and I've put it in [the {linkrot} package on GitHub](https://en.wikipedia.org/wiki/Link_rot). To install:
...
It can take a short while for the function to visit every link. To let us know it's working, the URL is printed to the console and then a dot is printed for every link that's been successfully visited (a bit like a progress bar).
...
The tibble includes a special standardised three-digit ['status code'](https://en.wikipedia.org/wiki/List_of_HTTP_status_codes) in the `response_code` column. These indicate whether contact was successful, with a specific reason. For example, 200 represents a typical success ('OK'), but you may be familiar with 404 ('not found') if you've visited a broken link before.
...
```{r webshot, fig.alt="A screenshot of the simple 404 page for the CRAN website, which can be seen when trying to access a URL that has no content. It says 'object not found' and that 'the requested URL was not found on this server'."}
library(webshot)
...
First we need all the post URLs, which luckily are all available from [the blog's homepage](https://www.rostrum.blog). The links returned are internal (like `2021/06/28/pixel-art/`), so we need to add on the `https://www.rostrum.blog/` bit). We also need to filter out any links that aren't posts (like the 'About' page).
...
So, this results tibble has `r nrow(results)` links from `r length(posts)` posts, or about `r round(nrow(results) / length(posts), 0)` links per post.
...
We've seen it in action, but how does the function work? I'm not claiming the approach is optimal, but it obviously worked for my needs.
...
I welcome any thoughts or suggestions, particularly around testing. I'd like to use this package as a way to learn proper HTTP testing and have found [rOpenSci's _HTTP Testing in R_ book](https://books.ropensci.org/http-testing/index.html) useful so far.
