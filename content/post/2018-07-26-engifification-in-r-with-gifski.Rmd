...
Great news: 'the fastest gif encoder in the universe' has been created. The [the `gifski` package for R is now in CRAN](https://cran.r-project.org/web/packages/gifski/index.html). It's a simple yet fast and powerful way to create gifs from within R, built by Jeroen Oom ([\@opencpu](https://twitter.com/OpenCPU)) for [ROpenSci](https://ropensci.org/).
