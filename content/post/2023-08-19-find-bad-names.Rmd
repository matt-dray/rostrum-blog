...
I wrote an R function to help identify where you've used a variable name that's already in use as a function name, like `c <- 1` or `head <- "x"`.
...
Using existing function names as variable names is a [code smell](https://en.wikipedia.org/wiki/Code_smell). Especially if they're frequently used functions from base R like `head`, `str`, `paste`,  etc. 
...
The output is what you get from `getparsedata(parse())`[^parse], filtered for the illegal names. Helpfully it shows you the exact row and column indices for where the string exists in the code you provided.

And of course you can just isolate the offenders.

```{r}
naughty$text |> unique() |> sort()
```

Yes, sometimes I use `df` as a name for temporary dataframes too.
...
[^rstring]: This is an 'R string', introduced in R version 4.0.0. It deals with escaping characters and using quotes within quotes. So `"x <- "y""` will error but `r"(x <- "y")"` will return `"x <- \"y\""`.  You can use symbols other than parentheses, such as curly braces, if your expression already contains parentheses itself.
