---
title: 'Art of the Possible: quantifying colour with magick'
author: 'Matt Dray'
date: '2018-11-25'
categories:
  - R
...
![](https://media.giphy.com/media/Dym2LTuxGJJSw/giphy.gif)
...
image_append(c(duplo, duplo_mapped))
```
...
image_append(c(reef, reef_mapped))
...
[By Uwe H. Friese from Wikimedia Commons, CC BY-SA 3.0.](https://commons.wikimedia.org/wiki/Butterfly#/media/File:Schmetterling_1a_neucc.jpg)

```{r butterfly, echo=FALSE}
butterfly_path <- "https://upload.wikimedia.org/wikipedia/commons/0/0c/Schmetterling_1a_neucc.jpg"

butterfly <- 
  image_read(butterfly_path) %>%
  image_scale(geometry = c("x600"))

butterfly_mapped <- image_map(
  image = butterfly,  # original image
  map = simple_cols  # colours to map on
)

butterfly_col_freq <- butterfly_mapped %>%
  count_colors() %>%
  left_join(
    as_tibble(cols_vec) %>%
      rownames_to_column() %>%
...
image_append(c(butterfly, butterfly_mapped))

kable(butterfly_col_freq)
```
...
image_append(c(neon, neon_mapped))
