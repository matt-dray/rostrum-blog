...
So each row is an 'incident' to which the brigade were called and there's 5352 of them recorded in teh dataset. There are 25 columns for variables relating to incident, including when it was, what it was and where it was.

## Explore the data

This post isn't about the dataset, but it's worth having a closer look at it. I've added an interactive table below so you can search for incidents, creatures, locations and the nominal cost of the callout.
...
# build interactive table
  ...
  mutate(  # allows for dropdown on interactive table
    ...
    IncidentNominalCost = ifelse(
      ...
      yes = NA, no = as.numeric(IncidentNominalCost)
    )
  ) %>% 
  select(
