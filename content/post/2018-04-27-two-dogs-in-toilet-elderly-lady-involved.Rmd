...
Animals get stuck in weird places. Oh, and the `sf` package in R can be used to convert co-ordinates for interactive mapping.
...
I've got some data with eastings and northings. It's [animal rescue incidents](https://data.london.gov.uk/dataset/animal-rescue-incidents-attended-by-lfb/resource/e6211993-9ea2-4ed4-9378-7344653e9c31) attended by the `r emo::ji("fire_engine")` [London Fire Brigade](https://www.london-fire.gov.uk/incidents/) `r emo::ji("fire_engine")` from the excellent [London Data Store](https://data.london.gov.uk/). In their words:
...
<p>
<p>
Some of the more... 'unique' entries:
...
The `leaflet()` function expects latitude and longitude, not eastings and northings. There's a number of ways to make the conversion, but I'm going to use the [`sf` ('special features') package](https://github.com/r-spatial/sf).
...
So now we can plot the data. The `addMarkers()` function in `leaflet` will accept our `geometry` list-column and plot the latitude and longitude of each point.
...
<p>
<p>
Mostly I wrote this for my own reference. Feel free to discuss anything in the comments.

My main advice? Keep an eye on your pets.
...
* [Spatial Data in R: New Directions](https://edzer.github.io/UseR2017/#sf-handling-real-data) by Edzer Pebesma
...
* [Should I learn sf or sp for spatial R programming?](http://www.seascapemodels.org/rstats/2018/03/23/should-I-learn-sp-or-sf.html) by Seascape Models
