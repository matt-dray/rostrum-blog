...
```{r tidylog-example}
penguin_pipe <- penguins %>%
  filter(species %in% c("Chocolate", "Pingu")) %>% 
  group_by(species) %>% 
  summarise(`mean_mass_g` = mean(body_mass_g, na.rm = TRUE)) %>% 
  mutate(`mean_mass_g` = round(`mean_mass_g`, 1))
```
...
And here's how it looks after highlighting the code and selecting 'debug pipeline in browser' or 'burst pipes' from the RStudio addins menu:
