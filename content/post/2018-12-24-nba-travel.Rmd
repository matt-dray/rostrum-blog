...
We can extrac the table with its xpath, which is like an 'address' for HTML content. You can use the 'inspect' function in your browser or [the selectorgadget browser extension](https://selectorgadget.com/) to get this information.
...
So fans of the Portland Trail Blazers have the furthest total distance to travel to each of the other teams in the Northwest: over 9000 km. That's about teh same distance as London to Johannesburg in a straight line. Bear in mind these totals are only the travel in one direction as well.
...
1. [Got an API key](https://cloud.google.com/maps-platform/)
1. Scraped and wrangled data [with `rvest`](https://CRAN.R-project.org/package=gmapsdistance) and other packages
1. Mapped it with [the `leaflet` package](https://cran.r-project.org/web/packages/leaflet/index.html)
1. Queried the Google Maps API [with `gmapsdistance`](https://CRAN.R-project.org/package=gmapsdistance)
1. Summarised the output
1. Were impressed by the thought of any of these team's fans actually driving these distances
