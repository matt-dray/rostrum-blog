...
You can create simple Twitter bots with R using GitHub Actions and {rtweet}. I made [\@londonmapbot](https://www.twitter.com/londonmapbot).
...
So, I've made [\@londonmapbot](https://www.twitter.com/londonmapbot): a completely unsophisticated proof-of-concept Twitter bot that continues my learning about GitHub Actions from [a previous post](https://www.rostrum.blog/2020/08/09/ghactions-pkgs/).

What does it do? It posts a satellite image of a random bit of Greater London (well, within the M25 motorway) every half hour. Some of these are quite pleasing to look at. Below is an example image from [an existing \@londonmapbot tweet](https://twitter.com/londonmapbot/status/1309199157530431489?s=20). Can you guess where it is?[^clue]

<div class="figure">
<img src="/post/2020-09-19-londonmapbot_files/mystery-location.jpg" alt="A satellite image of part of London, showing what looks like a large number of tennis courts, including some in stadia." width=600/>
<p class="caption"></p>
</div>

So how does this thing work? A scheduled action runs code to generate random coordinates, send them to the [Mapbox API](https://docs.mapbox.com/api/maps/#static-images), retrieve an image, and then post it via [the Twitter API](https://developer.twitter.com/en/docs), which is accessed through the R package [{rtweet}](https://docs.ropensci.org/rtweet/). The source code is [a single YAML file](https://github.com/matt-dray/londonmapbot) containing the action and [a single R script](https://github.com/matt-dray/londonmapbot/blob/master/londonmapbot-tweet.R) that generates the tweet and posts it.

# The components

Let's look at the how GitHub Actions, {rtweet} and Mapbox are used in londonmapbot.
...
An action can be triggered by an event, like a `git push` to your repo. You can also schedule it with a [cron job](https://simple.wikipedia.org/wiki/Cron). I took advantage of this for londonmapbot, which executes every half-hour.
...
The action runs an R script that generates content for a tweet and then posts it. This script makes use of the package [{rtweet}](https://docs.ropensci.org/rtweet/) by [Mike Kearney](https://mikewk.com/), which lets you interact with Twitter through R via its [API](https://developer.twitter.com/en/docs/twitter-api).
...
You basically modify a particular URL string to ask the API for what you want. For example, you can ask for a `300x200` pixel `satellite` image of the coordinates of `-123.1387` and `49.3037`, which is [Stanley Park in Vancouver, Canada](https://en.wikipedia.org/wiki/Stanley_Park):
