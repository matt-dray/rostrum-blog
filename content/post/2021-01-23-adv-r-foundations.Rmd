...
Our specialty (and only!) pizza is _pizza alla napoletana_, which is topped with mozzarella, tomatoes and anchovies.[^mozzarella]
...
knitr::kable(order)  # print as HTML table
```
...
knitr::kable(order_update)   # print as HTML table
```
...
knitr::kable(order_final)   # print as HTML table
```
...
[^mozzarella]: You're thinking: 'this cheese emoji `r emo::ji("cheese")` doesn't look like mozzarrella'. A white circle emoji `r emo::ji("white_circle")` looks more like it, but hard to see when the page background is white. And the buffalo emoji `r emo::ji("buffalo")` implies the pizza has buffalo meat on, maybe. I would probably try that.