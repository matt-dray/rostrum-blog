...
R is a game engine[^explain]; don't @ me[^fosstodon]. As such, R has the ability to [keep a persistent save state](https://r-pkgs.org/data.html#sec-data-persistent).
...
Obviously I've used this to make a concept R package: [{tamRgo}](https://github.com/matt-dray/tamRgo). It saves locally a 'blueprint' for a  [_Tamgotchi_](https://en.wikipedia.org/wiki/Tamagotchi)-like pet[^tama], which is read and updated when you interact with your pet in the R console.
...
Let's quickly check what our pet looks like with `see_pet()`. Its appearance depends on the species and the level, but newborns are pretty much a blob.

```{r eval=FALSE}
see_pet()
```

I'll paste here an image of how the pet looks in the console, since the blog doesn't render it particularly well.

<div class="figure">
<img src="/post/2022-10-29-tamRgo_files/y0.png" alt="Descriptive text." width="20%"/>
</div>

<details><summary>Click for console output.</summary>
```{r eval=FALSE}
#       
#  ███  
# █ █ █ 
...
Anyway, like I said, this is a for-fun concept that's nowhere near feature complete[^others]. If I wanted to improve it, what would I do?  Plenty, including:

* animated pixel graphics
* the ability to transfer a blueprint between machines, so your pet can live across multiple devices[^gist]
* a battle system like _Digimon_
* a more fun minigame in `play()` that grants more XP for greater skill
...
You can probably think of other ways to use `tools::R_user_dir()` for games in R. Maybe you could have a 'save state' that can be recalled each time a game is played. Let me know when you've made a new triple-A game for R and I'll add it to [the list](https://github.com/matt-dray/awesome-r-games).
