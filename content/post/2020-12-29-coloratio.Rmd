...
But I wondered: is it possible to programmatically select a high-contrast text colour to overlay the fill colours of a {ggplot2} barplot? You would want black text over white and vice versa, for example. It isn't a big deal to program this 'manually', but that's not fun.
...
This problem calls for a small package with some functions to derive colour contrast ratios. Introducing [{coloratio}](https://github.com/matt-dray/coloratio).
...
Like `coloratio::cr_get_ratio()`, you can pass two hex values or named colours to {savonliquide}'s `check_contrast()` function.

```{r}
savonliquide::check_contrast("blanchedalmond", "bisque2")
```

You get a more informative output than for `coloratio::cr_get_ratio()`: it provides results from testing against [additional accessibility checks in WCAG 2.1](https://www.w3.org/TR/WCAG21/#contrast-minimum) that involve larger text and more a stringent contrast threshold. 

The two provided colours above are so similar that they fail the tests across the board.
