...
John Darnielle is well-known for 

```{r album_count}
unique(songs_meta$album_name)
```

We can label the studio albums by scraping a list of them from [The Mountain Goats discography page on Wikipedia(https://en.wikipedia.org/wiki/The_Mountain_Goats_discography).
...
# read the webpage
wiki <- xml2::read_html("https://en.wikipedia.org/wiki/The_Mountain_Goats_discography")

# get borough names
studio_albums <- wiki %>% 
  rvest::html_nodes(css = ".plainrowheaders tr~ tr+ tr th") %>%
  rvest::html_text()
...
lyrics <- lyrics %>%
  ...
  ungroup() %>% 
  left_join(songs_meta_filtered, by = "song_name")

# check out a sample of lines
lyrics %>% 
  dplyr::sample_n(10) %>% 
  select(line, song_name, album_name)
```
...
lyrics_tidy %>%
  filter(album_name %in% c("All Hail West Texas", "Goths", "Beat the Champ", "The Sunset Tree")) %>%
  inner_join(sentiment_nrc, by = "word") %>%
  group_by(album_name) %>% 
  count(sentiment) %>% 
  arrange(album_name, desc(n))
```