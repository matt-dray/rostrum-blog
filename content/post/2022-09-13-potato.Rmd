...
in addition, we clearly need user input to decide what to do during the game. Most of the time, a user's hand is forced and they need to roll. But sometimes they have the choice to remove one orc at the cost of one potato.
...
If the cost to yeet an orc is equal-to or less-than the number of potatoes, we can elect to make the payment. This is expressed in the options provided on the command line.
...
Both of which require user input at the command line, resulting in a value stored to the `event` object. Note that hitting <kbd>Enter</kbd> results in an empty string (`""`).
