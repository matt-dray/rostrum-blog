...
The map is an inventory item for looking up the geography of the overworld. It looks like an abstraction, but it's actually pretty similar to the gameworld. Some folks have [stitched together all the overworld screens to make a megamap](https://www.reddit.com/r/pokemon/comments/lpttq8/put_together_all_the_images_to_create_a_gen_i/) so you can see the similarity.

You might be thinking that it looks like a schematic map---an abstraction of the actual geography---like the London underground map. In fact, the town map is pretty similar to the actual in-game world layout, as demonstrated by folks who have [stitched together all the overworld screens to make a megamap](https://www.reddit.com/r/pokemon/comments/lpttq8/put_together_all_the_images_to_create_a_gen_i/).
...
This is not magic. It is merely powered by the `locator()` function, which records the x and y location of a point clicked on a plot by the user.[^useless] The first argument is the number of clicks to collect.

For this bespoke function, we pass a character vector of place names. For this demo, that'll be Kanto's towns, cities and other places of interest. The function loops through the locations and requests you to click the corresponding point on the map.
...
Aside: here's a simplified version of what's going on when you use `locator()`. Make a plot, call the function and then the top of the plotting window in RStudio says 'locator active' and your cursor becomes crosshairs. Clicking on the plot returns a list of the x and y coordinates within the plotting space.
...
During the clickfest, the `locate_points()` function has assembled the points lists into a data.frame with one row per location. The locations vector was passed as the rownames of the dataframe as well.
...
It might also be fun to do a minimal map of the cities where the points are coloured according to the name of the city. You may have noticed that the city names are all fancy colour names (viridian, fuchsia, etc), so let's use them. Well, except Pallett, for which can just use a mix of all colours, i.e. white.
...
Anyway, these 'maps' are the first steps to create something more exciting. For now they demonstrate the point (literally, lol). Plus they fulfil my belated submission for day one of [the #30DayMapChallenge](https://github.com/tjukanovt/30DayMapChallenge) ('points'), I guess?
...
But it means we can say some really profound (well... weird) stuff like it's about half a kilometre from Cerulean City to Cinnabar Island. Maybe that's true in the context of the game's dimensions, but it's... underwhelming.

Was this distance-conversion exercise worthwhile? No. But it might strengthen my belated submission to day two of [the #30DayMapChallenge](https://github.com/tjukanovt/30DayMapChallenge) ('lines')?
