...
Use R to interactively extract arbitrary coordinates from an image. I extracted them for the cities of Kanto from the first-generation of _Pokémon_ games.
...
[insert gif of manual clicking process]

The output is a data.frame of the coordinates with one row per location. The locations vector is passed as the rownames of the dataframe.
...
text(
  kanto_pts$x, kanto_pts$y,
  row.names(kanto_pts),
  cex = ifelse(kanto_pts$city, 0.9, 0.8),
  pos = c(rep(4, 6), 2, 4, 2, rep(4, 7)),
  family = "Arial Narrow"
)
```

It might be fun to do a minimal map of the cities where the points are coloured according to the name of the city. You may have noticed that the city names are all fancy colour names, so let's use them. Well, except Pallett, which can be a mix of all colours.
