...
2. click 'generate access token' to generate an access token
3. after `install.packages("geniusr"); library(geniusr); geniusr::genius_token()` you'll be promopted to enter the access token (if you need to change this, you can use `geniusr::genius_token(force = TRUE)` to be re-prompted for a new token, which you can regenerate from the [Genius site](https://genius.com/api-clients)
...
for (i in 1:length(goat_song_urls)){
  ...
  try_genius <- try(geniusr::scrape_lyrics_url(goat_song_urls[i]))
...
beepr::beep(8)  # alert on completion
```
