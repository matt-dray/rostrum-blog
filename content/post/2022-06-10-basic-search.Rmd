...
I've experimented with simple breadth-first search for [{r.oguelike}](https://github.com/matt-dray/r.oguelike), a work-in-progress  game-in-a-package for R. This means enemies can pathfind and chase down the player character.
...
I could let the enemy wander randomly on each turn, which is easy to implement, but boring and unrealistic. Far better would be to introduce some kind of pathfinding via an algorithm, which would make enemies head toward the player character to engage in battle.
...
Of course, it'll need to consider:
...
* allowing different enemy classes to move differently, e.g. randomly, or when the player is within a certain distance, or they begin hunting immediately
