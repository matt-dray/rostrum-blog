...
This made it straightforward to prepare a little Quarto doc with chunks powered by the 'webr' engine, which I deployed to the web via [Netlify](https://www.netlify.com/).[^netlify] So now you _can_ tinker with the example I gave in the blogpost.

You can [visit that live page](https://webr-parse-test.netlify.app/) or see the underlying [source on GitHub](https://github.com/matt-dray/webr-parse-test). I've also embedded the post below.[^iframe]

<iframe width="700" height="400" src="https://webr-parse-test.netlify.app/" title="An interactive web page that lets the user alter R code in the browser."></iframe>
...
In fact, this blog may eventually switch from {blogdown} to Quarto to take advantage of WebR. It'll be a pain to convert old posts, but I'm glad I missed the earlier {blogdown}-to-{distill} bandwagon!
...
[^iframe]: An iframe containing a link to a Quarto site is pretty garbage, so probably best just to visit the site.