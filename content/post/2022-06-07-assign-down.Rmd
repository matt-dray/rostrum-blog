...
First I'll write a demo script to temporary file. It assigns the values of `1` and `2` to `x` and `y`, respectively, then adds them together.
...
Now to define the function. It reads our script file, finds the down pipes, substitutes them, returns them back to the expression from whence they came, then executes the script.
...
Note that this function won't work in an interactive session; it takes the filepath to a script as its input. But that's okay: I think `<-` haters are often computer-scientist types and 1337 h4x0rz who never sully their code by playing around in filthy IDEs and notebooks. Obviously we should emanate them.
...
[^take]: [The latest](https://twitter.com/ryxcommar/status/1533522184299331584)?