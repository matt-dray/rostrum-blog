...
I've written a function called `create_brickr_player()` that lets you build a soccer player and select the brick colours for the shirt, socks, and much more. It's an exercise in being able to create the same model but change the brick colours with minimum fuss. 

The (completely untested) function lives in my package of misc functions, {dray}, which can be installed it from GitHub. 
...
The function is simple. It helps you create a dataframe that specifies the location and colour of individual bricks on successive 2D planes to build up a 3D model.
...
How do you know which numbers encode which colours? You can access the list from the `lego_colors` data frame in the {brickr} package.

```{r}
select(lego_colors, brickrID, Color, hex)  # colour codes
```

So '1' is white, '2' is black and so on. Timpe selected this set of colours to match the colours available from Lego sets. 
...
![A gif of a 3D Lego model of Man City soccer player Raheem Sterling](/post/2019-05-31-make-a-brickr-soccer-player_files/raheem.gif)
...
The obvious next step would be to allow for features of the plan to change. You could set an argument for `player_height` and add or remove layers from the plan to make the final model taller or shorter. You could have multiple hairstyles. You could 


[^timpe]: Not an official product, but Timpe actually now works at Lego.
