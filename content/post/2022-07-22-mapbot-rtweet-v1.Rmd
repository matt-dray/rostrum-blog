...
Version 1.0 of the {rtweet} package [has been released with breaking changes](https://ropensci.org/blog/2022/07/21/rtweet-1-0-0/). I've updated the R script of [londonmapbot](https://twitter.com/londonmapbot) so that the scheduled GitHub Action doesn't fail.
...
I made two major changes to [the londonmapbot R script](https://github.com/matt-dray/londonmapbot): 
...
I also made a third change---to specify the filetype of the downloaded MapBox image---that will be relevant if you forked londonmapbot or if your tweets use a downloaded image.

## 1. Authentication

You need to authenticate with Twitter API before a tweet can be posted.
...
There are now three options for passing tokens and keys, depending on the need: `rtweet_app()`, `rtweet_user()` and `rtweet_bot()`. The last of these is what we need, since it:

> authenticates a bot that takes actions on behalf of an app [which] is most appropriate if you want to create a Twitter account that is run by a computer, rather than a human
...
So, the code was switched to this:
...
Maybe we could infer something from the average colour of the image ([I wrote about this before](https://www.rostrum.blog/2021/07/23/london-colour/)) or maybe predict what the terrain is (we've plenty of training data from old londonmapbot tweets).
...
I noticed that the absence of an explicit file extension was causing an error in the execution of the R script under {rtweet} v1.0. So I made sure to change the code to this:
...
This post may not have impacted you if you aren't the in niche user group of 'people who may have forked or used a repo template for londonmapbot to adapt and make their own Twitter bot based on {rtweet} and GitHub Actions, or otherwise used it as inspiration to create their own bot'.[^join]
