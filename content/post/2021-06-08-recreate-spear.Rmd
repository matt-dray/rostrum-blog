---
title: "Recreating Spear's #CottonViz plot in base R"
author: Matt Dray
date: '2021-06-08'
slug: recreate-spear
categories:
...
<p class="caption">Progression of plot creation, finishing with the original.</p>
</div>

# \#CottonViz 

The [Royal Statistical Society](https://rss.org.uk/) (RSS) has [challenged people](https://rss.org.uk/news-publication/news-publications/2021/section-group-reports/mary-eleanor-spear-dataviz-competition-for-childre/) to recreate or remix [Mary Eleanor Spear's](https://en.wikipedia.org/wiki/Mary_Eleanor_Spear) visualisation of cotton supplies in the United States in the 1940s. Spear might be best known for the box plot, which she originated as the range plot in her book [Charting Statistics](https://archive.org/details/ChartingStatistics_201801). 

Here's her original cotton visualisation:
...
<p class="caption">Spear's original visualisation</p>
</div>

I thought it would be interesting to recreate it using only R's built-in base graphics. This might be a nice demo of zero-dependency plotting for R users who are more familiar with {ggplot2}. 

Long-story short, here's my attempt:
...
As an aside, I've also been inspired of late by [#RecreationThursday on Twitter](#recreationthursday). It's a community challenge to [recreate an art piece](https://twitter.com/_natalie_oshea/status/1400526683170541570?s=20) selected each fortnight by a rotating curator.
...
* [Routed Gothic](https://webonastick.com/fonts/routed-gothic/) for the majority of the text
* [Hussar Bold Condensed](https://www.ffonts.net/Hussar-Bold-Condensed.font) for the title

I'm using macOS Big Sur and R v4.0.5. It was sufficient for me to install these fonts on my system and invoke them by name as you'll see through the rest of this walkthrough. Your mileage may vary.

## Constants

I've set a few values as objects so they can be reused and changed more easily without having to copy-paste. Note that I've used the convention here that constants are `ALL_CAPS`, so they're easier to spot in the plotting code.

<details><summary>Click for constants</summary>
```{r eval=TRUE}
# Constants: general
CEX       <- 0.8
YDIV      <- 1000
...
I would normally do iterative things with the `map()` family of functions from the {purrr} package, so I welcome suggestions on appropriate use of the various base `*apply()` functions in this scenario.

The final thing is the arrows that point to the lines from the labels. Base R has a handy `arrows()` function to which you supply start and end coordinates and parameters for the arrowhead.
...
barplot(
  ...
  col = BLACK,
  density = rep(c(BP_HATCH_HI, 0, BP_HATCH_LO), 7),
...
Note that this layer of hatching is only required in the bottom and top bars because they will end up being crosshatched.
...
And now we have both layers of hatching:
...
Awkwardly, the first x-axis label in the bar plot had to be created manually because it's placed under the origin point in the original chart, rather than under the bar itself.
...
There were a few 'hacks' I employed to try and match the original image.
...
* I haven't really optimised the code or tried to tidy it up; there may be some unecessary that were part of development that don't need to be in the final script.
...
* I've used a lot of manual adjustments to get the chart elements in the right place. That's fine for a one-off chart like this, but isn't that useful for making this code more generic.
* R's built-in line dashes aren't quite the same as Spear's, but I think they're close enough.
* Of course, we can save our plot with maximum resolution, etc, but for fun we can invoke some of the artefacts that are present in Spear's plot with some help from the {magick} image-processing package (e.g. see below for how to add blur).

```{r}
library(magick)  # install from CRAN

image_blur(
  image_read(""),  # path to where you wrote the image
