...
<p class="caption">As in 'R to English', of course.</p>
</div>
...
'Speakable' means I should be able to read an R expression to you as an English sentence, rather than reading out individual characters.
...
* map the most common R operators
* use commonly-used English translations for R operators
* work on a simple one-to-one, left-to-right mapping of R to English
* keep the API simple
* begin a practical approach for addressing issues raised in Amelia's presentation 
...
You can install the package from GitHub, which is in development version 0.0.0.9005 at the time of writing. This post discusses current functionality. Things may change.
...
An important part of this process is parsing R expressions and recognising tokens using the `lintr::get_source_expressions()` function. For example, it detects `<-` as the special token `LEFT_ASSIGN`.
...
Put simply, {r2eng} hijacks this process and then adds the step of mapping each tokens to English terms, before recombining the text into a sentence.
...
First I'll attach the package havign installed it.
...
```{r translate-str}
library(magrittr)  # attached for pipes
"data<-1+1" %>%
...
You can find the source for this at [try-r2eng](https://github.com/matt-dray/try-r2eng) GitHub repo and you can [read one of my posts](https://www.rostrum.blog/2019/08/25/holepunch-drake/) on how to set up Binder using [Karthik Ram's {holepunch} package](https://karthik.github.io/holepunch/).
...
* expand the package non-English languages ({r2es}, {r2fr}, etc)
* etc
