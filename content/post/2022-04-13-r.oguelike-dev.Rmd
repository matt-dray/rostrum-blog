...
* randomly-generated dungeons (a different playing surface every time)
...
Top-tip: improve immersion by changing your console colour palette to dark mode, naturally.
...
So we know we won because the status message changed to `You win!` and a note of how many hit points we lost: `(-3 HP)`. Concurrently the HP in the inventory bar has reduced by that amount[^hp].
...
[^hp]: Haha, yes, 10 minus 3 is not 6. This is a little bug I discovered when writing this blog post. Maybe I'll fix it eventually.
[^symbols]: Why are they `$` and `a` on the map, but `G` and `A` in the inventory bar? Good question.