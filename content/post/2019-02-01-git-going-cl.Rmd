...
When you start, the prompt will show the tilde `~` by default. This is shortcut symbol for your 'home' directory, which on a Mac will be set to the path `/Users/<your-username>`.
...
You can see a description of the command and the list of available flags useing the `man` command. For example, try `man ls` to learn about the list command. Sometimes the contents of the man file don't fit the screen; keep hitting return unti lthe bottom, or tap `q` to quit.
