...
![Example of a Terminal window and commands](/post/2019-02-01-git-going-cl_files/terminal-example.png)
...
These are likely to be your most commonly-used bash commands. These will be useful when you start using the command line for version-control with Git.
...
What language are we going to use to communicate?  We'll be writing in our computer's default scripting langage, called [bash](https://en.wikipedia.org/wiki/Bash_(Unix_shell)). In this post we'll be looking at a few basic commands.
...
![Example of Nano running in Terminal for this blog's README file](/post/2019-02-01-git-going-cl_files/nano-example.png)

```
bros:important-notes luigi$ nano mushroom-top3.md
```
The prompt will disappear and you'll see a header saying `File: characters.md`. You're now editing this file with Nano. Start typing. The file extension `.md` means you can write using Markdown syntax, but you cold just as easily have specified a `.txt` text file, for example.
...
Notice the options at the bottom of the window. We can exit Nano with `^X` (control and X keys). You'll be asked if you want to save; hit `Y` for yes. Then you'll be asked for a `File name to write: characters.md`. You can edit the name or hit enter to confirm. The content is now saved.
...
Let's say we have a rogue text file in our home directory (`~`) that should really be in a subfolder of `Documents/`. The command `mv` allows us to move it. It's like dragging and dropping a file from one folder into another.

You can specify this in the form `mv <filepath> <filepath>` to move files to and from any path, or  or navigate with `cd` to the folder containing the file and use the form `mv <file> <filepath>`). The code below does the latter, using `cd` and `ls` to move between folders and show where the file is.
...
Well done for reaching the finish. So we should have enough knowledge now to navigate and manipulate folders. This will be extremely helpful when we start using Git commands at the command line in part two of the 'Git going' series.

![_I'm-a Luigi, number one (at the command line)_](https://media.giphy.com/media/Ive85Xdzhpg8E/giphy.gif)