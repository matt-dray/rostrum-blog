...
This takes a little while. There's probably faster methods, like maybe [the {pak} package](https://pak.r-lib.org/), but for now I just used what worked.
...
It's not particularly exciting to know that the top 5 are made up of base R functions like the assignment arrow (`<-`) and the dollar-sign (`$`) data accessor[^nzd] and the square bracket (`[`). We also don't really care about the package's internal functions. Let's filter out these packages and re-count
...
What about total calls by each package?
...
Use whatever dependencies you want, it's up to you. {itdepends} might be a useful tool for you to work out if you need all the dependencies you have. Other tools are out there; [I read recently](https://ashbaldry.github.io/2021-11-25-depcheck-r-dependency-checker/) about [Ashley Baldry's {depcheck} package](https://github.com/ashbaldry/depcheck), for example

It might be interesting to do this for all CRAN packages and their dependencies, but I don't have a personal CRAN mirror and I don't write particularly performant code.
