...
I wrote [an R package, {dehex}](https://github.com/matt-dray/dehex), that can help you 'read' a hex colour code by eye according to [David DeSandro's method](https://metafizzy.co/blog/read-color-hex-codes/).

# Hue are you?

[Hex codes](https://en.wikipedia.org/wiki/Web_colors) can be used to encode a colour as a succinct six-digit alphanumeric string, like #F4D82A.

These codes are written in hexadecimal (hence 'hex'): they can take the characters 0 to 9 and A to F, which encodes 16 possible values. Sixteen million colours can be encoded in this way.
...
That's easy enough to decipher, but what about #8ACD52 or #C0FFEE?[^words] You could copy these into a tool that returns colour information (many search engines can do this now) but they usually return a sample and not the colour's name.
...
For the more adventurous, you can use {dehex} to learn how to read a hex code with DeSandro's method.
...
For whatever reason, certain browsers struggle to render these graph outputs correctly in this blog post (seems fine in Firefox). In your console, the graphs will appear with neat 'continuous' bars instead of as individual blocks. Example from [the GitHub README](https://github.com/matt-dray/dehex/blob/main/README.md):

<div class="figure">
<img src="/post/2021-08-09-dehex_files/dh-graph-correct.png" alt="Output from the dehex package's dh_graph function, which shows a horizontal bar chart in the RStudio console with a light theme. The columns are labelled R, G, and B and are coloured red, green and blue. Above the plot is the three-digit colour hex code that graph is summarising." width="300px"/>
</div>
</div>
...
Now we repeat the process for [saturation](https://en.wikipedia.org/wiki/Colorfulness). This time we're going to compare the range of RGB values. This is easier because there's only three (['saturate'd, 'washed' and 'muted') plus grey (i.e. zero range)](https://speakerdeck.com/desandro/read-color-hex-codes?slide=50).
...
Finally, we do the same for the [lightness](https://en.wikipedia.org/wiki/Lightness) of the colour (['light', 'middle' and 'dark'](https://speakerdeck.com/desandro/read-color-hex-codes?slide=46). DeSandro says that you can total up the values, where higher values are lighter, but I've chosen to mark the mean value on the guide for lightness in {dehex} because I think it's easier to interpret from the graph format.
