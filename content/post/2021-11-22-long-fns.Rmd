...
It can be tricky to be succinct. Consider the base R function `suppressPackageStartupMessages()`[^suppress]: it's a whopping 30 characters, but all the words are important and something like `suppPkgStartMsg()` isn't so clear.

It [made me wonder](https://www.rostrum.blog/2021/08/27/zzz/#fn1): what's the longest function name in R?[^luke]

But! It seems tricky to and time consuming to find the longest function name from _all_ R packages. CRAN alone has over 18,000 at time of writing.
...
For fun, we can use this as an excuse to demo [the dog's balls approach](https://milesmcbain.micro.blog/2021/06/23/how-youd-fix.html) to function-writing in R v4.1.[^underscore]
...
Of the `r nrow(base_fns)` objects, a quick histogram shows that the most frequent character length is under 10, with a tail stretching out to over 30.
...
hist(
  base_fns$nchar,
  main = "Character length of base-package object names",
...
So there are four objects with names longer than `suppressPackageStartupMessages()`, which are rarely used as far as I can tell. The longest is `aspell_write_personal_dictionary_file()`, which has 37 (!) characters. It's part of the spellcheck functions in {utils}.
...
What about shortest functions? There are many one-character functions.
...
One method is to get the a vector of the package names with the `tidyverse_packages()` function and passing it to `p_load()` from {pacman}, which prevents the need for a separate `library()` call for each one.[^pac]
...
[^suppress]: [I wrote a whole post about this recently](https://www.rostrum.blog/2021/08/27/zzz/).
