...
Is there a way to stop this from happening? Maybe there's a setting in RStudio or something?

Well, I thought I'd hack together a little function that can be run on startup and warn me if I have multiple RStudio sessions that are opened from the same RStudio Project file. It's called `check_rproj_dupes()`.
...
6. Display any matches in a warning message
...
How would you use this? You can add it to your hidden .Rprofile file, which is a place that you can store code that runs whenever RStudio is started.[^rprofile] Perhaps the easiest way to open it is with `usethis::edit_r_profile()`. Then you can paste in all the code in the block above.[^gist]
...
So hopefully now I will be less confused when trying to manage my RStudio sessions.
