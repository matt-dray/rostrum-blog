...
Cut to the chase: I made a tiny package called {gpx3d}. For now it does what I want it to do.
...
There are also two demo files:

* `segment.gpx`, a shorter, edited version of the route used in this blogpost, which you can access with `system.file("extdata", "segment.gpx", package = "gpx3d")` after installing the package
* `gpx_segment`, an sf-class data.frame that's the result of using the `extract_gpx3d()` on the internal `segment.gpx` file
...
To use `extract_gpx3d()`, simply pass a path to a GPX file. I've chosen a 10 km run I took on Christmas morning, which I downloaded from Apple Health and stored locally.[^demo]
...
You can take this dataset away and do other stuff with it, like create a lat-long plot of the route (left), or the elevation over time (right).
...
Cute, especially the elevation plot in this case. 
