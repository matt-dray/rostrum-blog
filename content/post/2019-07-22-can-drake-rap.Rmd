...
The [{drake}](https://github.com/ropensci/drake) package records file interdependecies in your analysis. When files are changed, {drake} only re-runs the parts of your analysis that need to be re-run. This saves time and reduces error.

I think {drake} has potential for [Reproducible Analytical Pipelines (RAP)](https://ukgovdatascience.github.io/rap-website/), an automated approach to producing UK government statistics that minimises error and speeds production.
...
The demo code is on [GitHub](https://github.com/matt-dray/drake-egg-rap), along with an [HTML version of the demo report](https://matt-dray.github.io/drake-egg-rap/egg-report.html). Here's a screenshot:

![The demo report](/post/2019-07-22-can-drake-rap_files/demo-report-page.png)

The rest of ths post explains the steps in the demo:
...
![](/post/2019-07-22-can-drake-rap_files/dependency-static.png)

The visualisation is shape-coded by target type (e.g. functions are triangles) and colour-coded to show whether everything is up-to-date (e.g. green if it is). Arrows show the file dependencies (e.g. both the `clean_data` fcuntion and `raw_data` file come together to produce the `data` object).

## 4. Make the plan {#make}

Having satisfied that the plan looks correct, running the plan is as simple as:

```{r eval=FALSE}
make(plan)
```

This runs all the instructions laid out in the `plan` object. All the commands are run on the corresponding targets. 

Where do the outputs go? In our case, we specifically asked for the HTML report -- generated from the R Markdown file -- to be written to the `docs/` folder. The other objects are cached in a `.drake/` folder so they can be retrieved without needing to be re-created.

So, re-running the plan straight away again does... nothing. No code needs to be re-executed if nothing's changed! The outputs, and the cache in `.drake/`, are fully up to date.
...
![](/post/2019-07-22-can-drake-rap_files/dependency-outdated-static.png)
...
Of course, this shoul dbe considered the starting point, since [{drake} does a lot more](https://ropenscilabs.github.io/drake-manual/).
