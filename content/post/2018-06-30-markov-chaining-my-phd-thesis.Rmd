...
Because I'm helpful I've created a text file version of my thesis. You can get it raw from my [draytasets GitHub repo](https://github.com/matt-dray/draytasets).
...
# replace common punctuation with 
phd_text <- gsub(".", " .", phd_text, fixed = TRUE)
phd_text <- gsub(",", " ,", phd_text, fixed = TRUE)
phd_text <- gsub("(", "( ", phd_text, fixed = TRUE)
phd_text <- gsub(")", " )", phd_text, fixed = TRUE)

# split into single tokens where the separator is a space
...
We're going to randomly seed the first start of each 'sentence' (a sequence of *n* words, where we specify *n*). We'll do this by supplying one of 200 unique values to the `set.seed()` function in turn. This seed starts the chain within the `markovchainSequence()` function.
...
I've selected some things from the output that basically lok like normal(ish) phrases. Simply rearrange these to build a thesis!
...
* Most studies could shift invertebrate communities [hang on, studies are affecting the thing ehy're investigating again!]
...
* Litter was affected by Wallace , Dordrecht [Who is this German guy who's interfering with my studies?]
