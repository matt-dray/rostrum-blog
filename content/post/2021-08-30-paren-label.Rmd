...
I wrote a novelty R function that inserts a comment after a closing parenthesis with the name of the function it belongs to. (These are called [biscuits](https://code-biscuits.github.io/), apparently.) It's available as an RStudio Addin from [the {blogsnip} package](https://github.com/matt-dray/blogsnip).
...
So what if we want to use those closing-paren labels, or 'biscuits', anyway? There doesn't seem to be an option in RStudio for that.
...
Maybe you could treat an expression as a string, then label the opening parens in order and the closing parens in reverse order. In this example, the letters correctly match the parentheses (uppercase for open-parens and lowercase for closing-parens):
...
For demonstration, we can use one of those simple examples from above as our input.
...
The `.parse()` function takes a string containing R code and returns a dataframe of its 'syntax tree'. In other words, it breaks the string into 'tokens' that are recognised as units of R code: function calls, assignment arrows, etc.[^r2eng]
...
It gets confused if there's already some closing paragraphs. It gets confused if you don't highlight enough code to capture all the opening and closing parens. It gets confused if you run it over more than one expression. It ignores anything other than a round parenthesis. Etc, etc.
...
[^r2eng]: This was the approach we used in [the {r2eng} package](https://github.com/matt-dray/r2eng/) to [convert R expressions to English sentences](https://www.rostrum.blog/2020/11/14/hello-r2eng/).