...
I wrote a novelty R function that inserts commenta after closing parentheses with the names of the functions they belong to. (These are called [biscuits](https://code-biscuits.github.io/), apparently.) It's available as an RStudio Addin from [the {blogsnip} package](https://github.com/matt-dray/blogsnip).
...
Install from GitHub with {remotes} as follows and then restart RStudio. {blogsnip} doesn't force you to install {lintr} and {styler}, so you'll have to do that too (if you haven't already).

```{r blogsnip, eval=FALSE}
install.packages(c("remotes", "lintr", "styler"))
...
It gets confused if there's already some closing parens. It gets confused if you don't highlight enough code to capture all the opening and closing parens. It gets confused if you run it over more than one expression. It ignores anything other than a round parenthesis. Etc, etc.
