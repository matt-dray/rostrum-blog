...
Run `ghd_download()` from [the {ghdump} package](https://github.com/matt-dray/ghdump){target='_blank'} to download all the GitHub repositories for a given user. Intended for archival purposes.
...
* it would be tedious to download the repos one-by-one from the GitHub interface

Need:

* to download all of my repos with one command
* be able to unzip them all en masse if I want to
...
* R has many file-handling functions taht will be helpful
...
It's dependent on [the handy {gh} package](https://github.com/r-lib/gh) by Gábor Csárdi, Jenny Bryan and Hadley Wickham for accessing the GitHub API.
