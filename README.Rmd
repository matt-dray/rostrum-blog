...
That's a post roughly every `r round((as.numeric(today() - min(posts$publish_date))) / nrow(posts))` days, or about `r round(((nrow(posts) / (as.numeric(today() - min(posts$publish_date)))) * 30), 1)` posts per month.

The latest post was published `r today() - max(posts$publish_date)` day(s) ago on on `r max(posts$publish_date)`.
