...
p <- posts %>%
  ...
  theme(
    ...
    axis.text.x = element_text(color = rostrum_green, size = 12),
...
There have been `r nrow(posts)` posts on the blog since the first was published on `r min(posts$publish_date)`.

That means a new post roughly every `r round((as.numeric(today() - min(posts$publish_date))) / nrow(posts))` days, or about `r round(((nrow(posts) / (as.numeric(today() - min(posts$publish_date)))) * 30))` posts per month.
...
Each vertical line in this chart is a published post:

```{r plot-out, fig.height=1, fig.width=10}
p
```

Here's an audio representation of the posts over time:

<html><audio controls><source src="README_files/audio/posts.wav" type="audio/wav"></audio></html>

_Stats last updated `r today()`._
